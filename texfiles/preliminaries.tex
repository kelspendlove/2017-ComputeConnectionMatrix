...
%Let $(X,\kappa)$ be a complex.  Let $(X,\preceq)$ be its face poset.  If $X'$ be an convex set in $(X,\preceq)$ then $(X',\partial_{X'})$ is a subcomplex.
%\end{prop}
%\begin{proof}
%We must show that $\partial_{X'}\circ \partial_{X'}=0$.  If $\xi,\xi''\in X'$ and $\xi'' \prec \xi$ then all $\xi'\in X$ such that $\xi'' \prec \xi' \prec \xi$ must be in $X'$ since $X'$ is an interval.  Thus~(\ref{cond:1}) and~(\ref{cond:2}) imply that $\sum_{\xi'\in X'} \kappa(\xi, \xi')\cdot \kappa(\xi',\xi'')=0$ which implies $\partial_{X'}^2=0$.
%\end{proof}
