...
%There is an assignment $\cL\colon Cell(J(L))\to \bLFC(\sL)$ and a graded-cell complex $f\colon X\to \sJ(\sL)$ is a graded basis for $\cL(f)$.
 
%$$Cell(\sJ(\sL))\to {\bf GCC(\sJ(\sL))} \to \bLFC(\sL)$$
}
\end{ex}
...
The homotopy category $\bK\bLFC(\sL,k)$ has $\sL$-filtered chain complexes as objects.
...
%
%\subsection{$\sP$-Graded Chain Complexes}\label{sec\colon lfc\colon grad}
%
%
%
...
\begin{cor}\label{prop:Lsplitting}
...
\]
%\begin{enumerate}
%\item $f(a)= (C^\oplus(a),\Delta(a))$
%\item for $a\leq b$ we have $f(b)/f(a) \cong (C^\oplus(b-a),\Delta(b-a))$.
%\end{enumerate}
\end{cor}
\begin{proof}
...
In this case we may interpret $\Delta$ as an $\sJ(\sL)$-graded boundary map on homology
\[
